...
double smoothStepInterp(double t) { return (t * t) * (3.0 - 2.0 * t); }

// rough approximation of pow(x,y)
double powFast(double x, double y) { return x / (x + y - y * x); }
...
osg::Vec3d computeWorld(osg::Node *node) {
  return node ? osg::Vec3d(0, 0, 0) * computeLocalToWorld(node)
              : osg::Vec3d(0, 0, 0);
}
...
struct ComputeLocalToWorld : osg::NodeVisitor {
  ...
  void accumulate(const osg::NodePath &path) {
    ...
    for (osg::NodePath::const_reverse_iterator i = path.rbegin();
      ...
      if (cam) {
        if (cam->getReferenceFrame() != osg::Transform::RELATIVE_RF ||
            cam->getParents().empty())
          break;
...
struct ManipTerrainCallback : public TerrainCallback {
  ...
  void onTileAdded(const TileKey &key, osg::Node *graph,
...
EarthManipulator::Action::Action(ActionType type, const ActionOptions &options)
    : _type(type), _options(options) {
  init();
}

EarthManipulator::Action::Action(ActionType type) : _type(type) { init(); }

void EarthManipulator::Action::init() {
  _dir = _type == ACTION_PAN_LEFT || _type == ACTION_ROTATE_LEFT
             ? DIR_LEFT
             : _type == ACTION_PAN_RIGHT || _type == ACTION_ROTATE_RIGHT
                   ? DIR_RIGHT
                   : _type == ACTION_PAN_UP || _type == ACTION_ROTATE_UP ||
                             _type == ACTION_ZOOM_IN
                         ? DIR_UP
                         : _type == ACTION_PAN_DOWN ||
                                   _type == ACTION_ROTATE_DOWN ||
                                   _type == ACTION_ZOOM_OUT
                               ? DIR_DOWN
                               : DIR_NA;
}

EarthManipulator::Action::Action(const Action &rhs)
    : _type(rhs._type), _dir(rhs._dir), _options(rhs._options) {
  // nop
}

bool EarthManipulator::Action::getBoolOption(int option,
                                             bool defaultValue) const {
  for (ActionOptions::const_iterator i = _options.begin(); i != _options.end();
       i++) {
    if (i->option() == option)
      return i->boolValue();
  }
...
int EarthManipulator::Action::getIntOption(int option, int defaultValue) const {
  for (ActionOptions::const_iterator i = _options.begin(); i != _options.end();
       i++) {
    if (i->option() == option)
      return i->intValue();
...
EarthManipulator::Action
    EarthManipulator::NullAction(EarthManipulator::ACTION_NULL);

static std::string s_actionNames[] = {
    "null",         "home",      "goto",        "pan",    "pan-left",
    "pan-right",    "pan-up",    "pan-down",    "rotate", "rotate-left",
    "rotate-right", "rotate-up", "rotate-down", "zoom",   "zoom-in",
    "zoom-out",     "earth-drag"};

static std::string s_actionOptionNames[] = {"scale-x",           "scale-y",
                                            "continuous",        "single-axis",
                                            "goto-range-factor", "duration"};
...
EarthManipulator::Settings::Settings()
    : osg::Referenced(), Revisioned(), _single_axis_rotation(false),
      _lock_azim_while_panning(true), _mouse_sens(1.0), _touch_sens(0.005),
      _keyboard_sens(1.0), _scroll_sens(1.0), _min_pitch(-89.9),
      _max_pitch(-1.0), _max_x_offset(0.0), _max_y_offset(0.0),
      _min_distance(1.0), _max_distance(DBL_MAX), _tether_mode(TETHER_CENTER),
      _arc_viewpoints(true), _auto_vp_duration(false), _min_vp_duration_s(3.0),
      _max_vp_duration_s(8.0), _orthoTracksPerspective(true),
      _terrainAvoidanceEnabled(true), _terrainAvoidanceMinDistance(1.0),
      _throwingEnabled(false), _throwDecayRate(0.05) {
  // NOP
}

EarthManipulator::Settings::Settings(const EarthManipulator::Settings &rhs)
    : osg::Referenced(rhs), Revisioned(rhs), _bindings(rhs._bindings),
      _single_axis_rotation(rhs._single_axis_rotation),
      _lock_azim_while_panning(rhs._lock_azim_while_panning),
      _mouse_sens(rhs._mouse_sens), _touch_sens(rhs._touch_sens),
      _keyboard_sens(rhs._keyboard_sens), _scroll_sens(rhs._scroll_sens),
      _min_pitch(rhs._min_pitch), _max_pitch(rhs._max_pitch),
      _max_x_offset(rhs._max_x_offset), _max_y_offset(rhs._max_y_offset),
      _min_distance(rhs._min_distance), _max_distance(rhs._max_distance),
      _tether_mode(rhs._tether_mode), _arc_viewpoints(rhs._arc_viewpoints),
...
void EarthManipulator::Settings::apply(osg::ArgumentParser &args) {
  bool boolval;
  double doubleval;
...
// expands one input spec into many if necessary, to deal with modifier key
// combos.
void EarthManipulator::Settings::expandSpec(const InputSpec &input,
                                            InputSpecs &output) const {
  int e = input._event_type;
  int i = input._input_mask;
  int m = input._modkey_mask;

  if (HASMODKEY(m, osgGA::GUIEventAdapter::MODKEY_CTRL)) {
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_LEFT_CTRL),
               output);
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_RIGHT_CTRL),
               output);
  } else if (HASMODKEY(m, osgGA::GUIEventAdapter::MODKEY_ALT)) {
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_LEFT_ALT),
               output);
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_RIGHT_ALT),
               output);
  } else if (HASMODKEY(m, osgGA::GUIEventAdapter::MODKEY_SHIFT)) {
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_LEFT_SHIFT),
               output);
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_RIGHT_SHIFT),
               output);
  } else if (HASMODKEY(m, osgGA::GUIEventAdapter::MODKEY_META)) {
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_LEFT_META),
               output);
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_RIGHT_META),
               output);
  } else if (HASMODKEY(m, osgGA::GUIEventAdapter::MODKEY_HYPER)) {
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_LEFT_HYPER),
               output);
    expandSpec(InputSpec(e, i, m & ~osgGA::GUIEventAdapter::MODKEY_RIGHT_HYPER),
               output);
  }

  // Always add the input so if we are dealing with a windowing system like QT
  // that just sends MODKEY_CTRL it will still work.
...
void EarthManipulator::Settings::bindMouse(ActionType actionType,
                                           int button_mask, int modkey_mask,
                                           const ActionOptions &options) {
  bind(InputSpec(osgGA::GUIEventAdapter::DRAG, button_mask, modkey_mask),
...
void EarthManipulator::Settings::bindMouseClick(ActionType action,
                                                ...
                                                int modkey_mask,
                                                const ActionOptions &options) {
  bind(InputSpec(EVENT_MOUSE_CLICK, button_mask, modkey_mask),
       Action(action, options));
}

void EarthManipulator::Settings::bindMouseDoubleClick(
    ActionType action, int button_mask, int modkey_mask,
    const ActionOptions &options) {
  bind(InputSpec(EVENT_MOUSE_DOUBLE_CLICK, button_mask, modkey_mask),
       Action(action, options));
}

void EarthManipulator::Settings::bindKey(ActionType action, int key,
                                         int modkey_mask,
                                         const ActionOptions &options) {
  bind(InputSpec(osgGA::GUIEventAdapter::KEYDOWN, key, modkey_mask),
       Action(action, options));
}
...
void EarthManipulator::Settings::bindScroll(ActionType action,
                                            ...
                                            int modkey_mask,
                                            const ActionOptions &options) {
  bind(InputSpec(osgGA::GUIEventAdapter::SCROLL, scrolling_motion, modkey_mask),
       Action(action, options));
}

void EarthManipulator::Settings::bindPinch(ActionType action,
                                           const ActionOptions &options) {
  bind(InputSpec(EarthManipulator::EVENT_MULTI_PINCH, 0, 0),
       Action(action, options));
}

void EarthManipulator::Settings::bindTwist(ActionType action,
                                           const ActionOptions &options) {
  bind(InputSpec(EarthManipulator::EVENT_MULTI_TWIST, 0, 0),
       Action(action, options));
}

void EarthManipulator::Settings::bindMultiDrag(ActionType action,
                                               const ActionOptions &options) {
  bind(InputSpec(EarthManipulator::EVENT_MULTI_DRAG, 0, 0),
       Action(action, options));
}

const EarthManipulator::Action &
EarthManipulator::Settings::getAction(int event_type, int input_mask,
                                      int modkey_mask) const {
  // Build the input spec but remove the numlock and caps lock from the modkey
  // mask.  On Linux these seem to be passed in as part of the modkeymask if they
  // are on.  So if you bind an action like SCROLL to a modkey mask of 0 or a
  // modkey mask of ctrl it will never match the spec exactly b/c the modkey mask
  // also includes capslock and numlock.
  InputSpec spec(event_type, input_mask,
                 modkey_mask & ~osgGA::GUIEventAdapter::MODKEY_NUM_LOCK &
                     ~osgGA::GUIEventAdapter::MODKEY_CAPS_LOCK);
...
void EarthManipulator::Settings::setAutoViewpointDurationLimits(
...
void EarthManipulator::ctor_init() {
...
EarthManipulator::EarthManipulator(const EarthManipulator &rhs)
    : osgGA::CameraManipulator(rhs), _last_action(ACTION_NULL),
      _last_event(EVENT_MOUSE_DOUBLE_CLICK), _time_s_last_event(0.0),
      _frameCount(0), _settings(new Settings(*rhs.getSettings())),
...
void EarthManipulator::configureDefaultSettings() {
  ...
  //_settings->bindMouse( ACTION_EARTH_DRAG,
  ...
  _settings->bindMouse(ACTION_ROTATE,
  ...
  _settings->bindMouseDoubleClick(
      ACTION_GOTO, osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON, 0L, options);

  // double click the right button (or CTRL-left button) to zoom out to a point
  options.clear();
  options.add(OPTION_GOTO_RANGE_FACTOR, 2.5);
  _settings->bindMouseDoubleClick(
      ACTION_GOTO, osgGA::GUIEventAdapter::RIGHT_MOUSE_BUTTON, 0L, options);
  _settings->bindMouseDoubleClick(ACTION_GOTO,
                                  osgGA::GUIEventAdapter::LEFT_MOUSE_BUTTON,
...
void EarthManipulator::applySettings(Settings *settings) {
  if (settings) {
    _settings = settings;
  } else {
    configureDefaultSettings();
  ...
  double new_pitch = osg::clampBetween(old_pitch, _settings->getMinPitch(),
                                       _settings->getMaxPitch());
...
EarthManipulator::Settings *EarthManipulator::getSettings() const {
  return _settings.get();
}

void EarthManipulator::reinitialize() {
  _distance = 1.0;
  _viewOffset.set(0, 0);
...
bool EarthManipulator::established() {
  ...
  if (!_homeViewpoint.isSet()) {
    if (_pendingViewpoint.isSet()) {
      setHomeViewpoint(_pendingViewpoint.get(),
                       _pendingViewpointDuration.as(Units::SECONDS));
    }
      ...
      vp.range()->set(_srs->getEllipsoid()->getRadiusEquator() * 3.0,
                      Units::METERS);
      ...
      vp.focalPoint() = GeoPoint(_srs.get(), profile->getExtent().getCentroid(),
                                 ALTMODE_ABSOLUTE);
      vp.heading()->set(0.0, Units::DEGREES);
  ...
  if (!_pendingViewpoint.isSet()) {
    ...
    setViewpoint(_pendingViewpoint.get(),
                 _pendingViewpointDuration.as(Units::SECONDS));
  }
...
bool EarthManipulator::createLocalCoordFrame(
    const osg::Vec3d &worldPos, osg::CoordinateFrame &out_frame) const {
  if (_srs.valid()) {
    osg::Vec3d mapPos;
...
void EarthManipulator::setNode(osg::Node *node) {
  // you can only set the node if it has not already been set, OR if you are
  // setting it to NULL. (So to change it, you must first set it to NULL.) This
  // is to prevent OSG from overwriting the node after you have already set on
  // manually.
...
osg::Node *EarthManipulator::getNode() { return _node.get(); }

osg::Matrixd EarthManipulator::getRotation(const osg::Vec3d &point) const {
  // The look vector will be going directly from the eye point to the point on
  // the earth, so the look vector is simply the up vector at the center point
  ...
  if (osg::equivalent(dot, 1.0)) {
    // We are looking nearly straight down the up vector, so use the Y vector
    // for world up instead
...
void EarthManipulator::setViewpoint(const Viewpoint &vp,
  ...
  if (!established()) {
    ...
    // Fill in any missing end-point data with defaults matching the current
    // camera setup. Then all fields are guaranteed to contain usable data
    // during transition.
    ...
    if (duration_seconds > 0.0) {
      ...
      startWorld =
          _setVP0->getNode(startNode) ? computeWorld(startNode.get()) : _center;
      ...
      // calculate the total distance the focal point will travel and derive an
      // arc height:
      ...
      if (_setVPArcHeight > 0.0) {
        // if we're arcing, we need separate coefficients for the up and down
        // stages
        ...
        _setVPAccel =
            fabs(dh2) <= 1.0 ? 0.0 : dh2 > 0.0 ? log10(dh2) : -log10(-dh2);
        if (fabs(_setVPAccel) < 1.0)
          _setVPAccel = 0.0;
      }
...
double EarthManipulator::setViewpointFrame(double time_s) {
  if (!_setVPStartTime.isSet()) {
    ...
    osg::Vec3d newCenter = _srs->isGeographic()
                               ? nlerp(startWorld, endWorld, tp)
                               : lerp(startWorld, endWorld, tp);

    // Calculate the delta-heading, and make sure we are going in the shortest
    // direction:
    ...
    if (d_azim.as(Units::RADIANS) > osg::PI)
      ...
      d_azim = d_azim + Angle(2.0 * osg::PI, Units::RADIANS);
    double newAzim =
        _setVP0->heading()->as(Units::RADIANS) + tp * d_azim.as(Units::RADIANS);

    // Calculate the new pitch:
    Angle d_pitch = _setVP1->pitch().get() - _setVP0->pitch().get();
    double newPitch =
        _setVP0->pitch()->as(Units::RADIANS) + tp * d_pitch.as(Units::RADIANS);

    // Calculate the new range:
    Distance d_range = _setVP1->range().get() - _setVP0->range().get();
    double newRange = _setVP0->range()->as(Units::METERS) +
                      d_range.as(Units::METERS) * tp +
                      sin(osg::PI * tp) * _setVPArcHeight;

    // Calculate the offsets
    osg::Vec3d offset0 =
        _setVP0->positionOffset().getOrUse(osg::Vec3d(0, 0, 0));
    osg::Vec3d offset1 =
        _setVP1->positionOffset().getOrUse(osg::Vec3d(0, 0, 0));
    ...
    if (t >= 1.0) {
      ...
      // If this was a transition into a tether, keep the endpoint around so we
...
void EarthManipulator::setLookAt(const osg::Vec3d &center, double azim,
  ...
  pitch =
      osg::clampBetween(pitch, osg::DegreesToRadians(_settings->getMinPitch()),
                        osg::DegreesToRadians(_settings->getMaxPitch()));

  _rotation = getQuaternion(azim, pitch);
}

void EarthManipulator::resetLookAt() {
  double pitch;
...
bool EarthManipulator::isSettingViewpoint() const {
  return _setVP0.isSet() && _setVP1.isSet();
}

void EarthManipulator::clearViewpoint() {
...
bool EarthManipulator::isTethering() const {
...
void EarthManipulator::collisionDetect() {
  if (!getSettings()->getTerrainAvoidanceEnabled() || !_srs.valid()) {
    return;
  }
  ...
  double r = osg::minimum(_srs->getEllipsoid()->getRadiusEquator(),
                          _srs->getEllipsoid()->getRadiusPolar());
  osg::Vec3d ip, normal;
...
bool EarthManipulator::intersect(const osg::Vec3d &start, const osg::Vec3d &end,
...
bool EarthManipulator::intersectLookVector(osg::Vec3d &out_eye,
  ...
  if (_mapNode.lock(mapNode) && mapNode->getTerrainEngine()) {
    ...
    if (lsi->containsIntersections()) {
      ...
      if (!_srs->isGeographic() ||
          GeoMath::isPointVisible(out_eye, out_target, R)) {
        success = true;
      }
    }

    if (!success) {
      // backup plan: intersect spheroid (if geocentric) or base plane (if
      // projected)
      if (_srs->isGeographic()) {
        osg::Vec3d i0, i1;
        unsigned hits = GeoMath::interesectLineWithSphere(
            out_eye, out_eye + look * 1e8, R, i0, i1);
        if (hits > 0) {
          ...
          // two hits? look vec intersects sphere (in two places)
            ...
            out_target =
                (out_eye - i0).length2() < (out_eye - i1).length2() ? i0 : i1;
      ...
      else // !_srs->isGeographic()
      {
        ...
        unsigned hits =
            GeoMath::intersectLineWithPlane(out_eye, out_eye + look, zup, i0);
        if (hits > 0) {
          out_target = i0;
          success = true;
        }
...
void EarthManipulator::home(double unused) {
  handleAction(ACTION_HOME, 0, 0, 0);
}

void EarthManipulator::home(const osgGA::GUIEventAdapter &,
...
void EarthManipulator::computeHomePosition() {
  if (getNode()) {
    ...
    osg::Vec3d eye =
        boundingSphere._center + osg::Vec3(0.0, -3.5f * boundingSphere._radius,
                                           boundingSphere._radius * 0.0001);

    setHomePosition(eye, boundingSphere._center, osg::Vec3d(0, 0, 1),
                    _autoComputeHomePosition);
  }
}

void EarthManipulator::init(const osgGA::GUIEventAdapter &,
...
void EarthManipulator::resetMouse(osgGA::GUIActionAdapter &aa,
                                  bool flushEventStack) {
  if (flushEventStack)
    flushMouseEventStack();
...
// Camera updates get called AFTER the scene gets its update traversal. So, if
// you have tethering enabled (or some other feature that tracks scene graph
// nodes), this will update the camera after the scene graph. This is important
// in order to maintain frame coherency and prevent "jitter".
//
// The reason we install/uninstall instead of just leaving it there is so we can
// support OSG's "ON_DEMAND" frame scheme, which disables itself is there are
// any update callbacks in the scene graph.
void EarthManipulator::updateProjection(osg::Camera *eventCamera) {
  ...
  if (eventCamera) {
    ...
    if (vp) {
      ...
      // For a perspective camera, remember the last known VFOV. We will need it
      // if we detect a switch to orthographic.
      if (!isOrtho) {
      ...
      // into proper x/y extents to simulate "zoom".
        ...
        // need to update the ortho projection matrix to reflect the camera
        // distance.
...
bool EarthManipulator::handle(const osgGA::GUIEventAdapter &ea,
  ...
  if (ea.getEventType() == osgGA::GUIEventAdapter::FRAME) {
    ...
    if (_node.valid()) {
      ...
      // in map-world space (e.g., ECEF) we need to then introduce this
      // transform at the very end (in getInverseMatrix()) to apply the final
      // view matrix.
      {
        osg::ref_ptr<MapNode> mapNode;
        if (_mapNode.lock(mapNode)) {
          _mapNodeFrame =
              osg::computeLocalToWorld(mapNode->getParentalNodePaths()[0]);
      ...
      if (_thrown) {
        ...
        _throw_dx = osg::absolute(_throw_dx) > osg::absolute(_dx * 0.01)
                        ? _throw_dx * decayFactor
                        : 0.0;
        _throw_dy = osg::absolute(_throw_dy) > osg::absolute(_dy * 0.01)
                        ? _throw_dy * decayFactor
                        : 0.0;

        if (_throw_dx == 0.0 && _throw_dy == 0.0)
          _thrown = false;
        else
          handleMovementAction(_last_action._type, _throw_dx, _throw_dy,
                               aa.asView());
      }
  ...
  // the camera manipulator runs last after any other event handlers. So bail
  // out if the incoming event has already been handled by another handler.
  if (ea.getHandled()) {
    return false;
  }
  ...
  if (isTethering() && ea.getEventType() != ea.FRAME) {
    ...
    if (atv.size() > 0) {
      ...
      const Action &action =
          _settings->getAction(eventType, buttonMask, modKeyMask);
  ...
  if (ea.isMultiTouchEvent()) {
    ...
    if (parseTouchEvents(te)) {
      ...
      // if a new push occurs we want to reset the dx/dy values to stop/prevent
      // throwing
  ...
  if (!handled) {
    ...
    case osgGA::GUIEventAdapter::RELEASE:
      ...
      if (_continuous) {
        ...
        if (_settings->getThrowingEnabled() &&
            (_time_s_now - _time_s_last_event < 0.05) &&
            (_throw_dx != 0.0 || _throw_dy != 0.0)) {
          _thrown = true;
          aa.requestRedraw();
          aa.requestContinuousUpdate(true);
          ...
          if (_mouse_down_event) {
            action = _settings->getAction(EVENT_MOUSE_CLICK,
                                          _mouse_down_event->getButtonMask(),
                                          _mouse_down_event->getModKeyMask());
    ...
    case osgGA::GUIEventAdapter::DOUBLECLICK:
      ...
      addMouseEvent(ea);
      if (_mouse_down_event) {
        action = _settings->getAction(ea.getEventType(),
                                      _mouse_down_event->getButtonMask(),
                                      _mouse_down_event->getModKeyMask());
        if (handlePointAction(action, ea.getX(), ea.getY(), aa.asView()))
          aa.requestRedraw();
    ...
    case osgGA::GUIEventAdapter::DRAG: {
      if (_pushed) {
        action = _settings->getAction(ea.getEventType(), ea.getButtonMask(),
                                      ea.getModKeyMask());
        addMouseEvent(ea);
    ...
    case osgGA::GUIEventAdapter::KEYDOWN:
      if (ea.getKey() < osgGA::GUIEventAdapter::KEY_Shift_L) {
        resetMouse(aa);
        action = _settings->getAction(ea.getEventType(), ea.getKey(),
                                      ea.getModKeyMask());
    ...
    case osgGA::GUIEventAdapter::SCROLL:
      resetMouse(aa);
      addMouseEvent(ea);
      action = _settings->getAction(ea.getEventType(), ea.getScrollingMotion(),
                                    ea.getModKeyMask());
      if (handleScrollAction(action,
                             action.getDoubleOption(OPTION_DURATION, 0.2)))
        aa.requestRedraw();
      handled = true;
      break;
    default:
...
public:
  CollectAllParentPaths(const osg::Node *haltTraversalAtNode = 0)
    ...
    // This is the same as the osg::CollectParentPaths visitor (which is not
    // exported) except for this node mask override to ensure that it will even
    // traverse nodes hidden via  node mask.
...
osg::NodePathList getAllParentalNodePaths(osg::Node *node,
...
void EarthManipulator::updateTether(double t) {
  ...
  if (_setVP1->getNode(node)) {
    // We use our getAllParentalNodePaths function instead of
    // tether_node->getParentalNodePaths() so that we can ensure even nodes
    // hidden via a node mask are traversed. Establish the reference frame for
    // the target node. If this fails, bail out.
    osg::Matrix L2W;
    ...
    // If we just called setViewpointFrame, no need to calculate the center
    // again.
    ...
    if (_settings->getTetherMode() == TETHER_CENTER) {
      if (_lastTetherMode == TETHER_CENTER_AND_ROTATION) {
        ...
        osg::Matrixd localToFrame(L2W *
                                  osg::Matrixd::inverse(_centerLocalToWorld));
      ...
      double sx = 1.0 / sqrt(L2W(0, 0) * L2W(0, 0) + L2W(1, 0) * L2W(1, 0) +
                             L2W(2, 0) * L2W(2, 0));
      double sy = 1.0 / sqrt(L2W(0, 1) * L2W(0, 1) + L2W(1, 1) * L2W(1, 1) +
                             L2W(2, 1) * L2W(2, 1));
      double sz = 1.0 / sqrt(L2W(0, 2) * L2W(0, 2) + L2W(1, 2) * L2W(1, 2) +
                             L2W(2, 2) * L2W(2, 2));
      L2W = L2W * osg::Matrixd::scale(sx, sy, sz);

      if (_settings->getTetherMode() == TETHER_CENTER_AND_HEADING) {
        // Back out the tetheree's rotation, then discard all but the heading
        // component:
        osg::Matrixd localToFrame(L2W *
                                  osg::Matrixd::inverse(_centerLocalToWorld));
        double azim = atan2(-localToFrame(0, 1), localToFrame(0, 0));
...
bool EarthManipulator::serviceTask() {
  if (_task.valid() && _task->_type != TASK_NONE) {
...
bool EarthManipulator::isMouseClick(
    const osgGA::GUIEventAdapter *mouse_up_event) const {
  if (mouse_up_event == NULL || _mouse_down_event == NULL)
    return false;

  static const float velocity = 0.1f;

  float dx =
      mouse_up_event->getXnormalized() - _mouse_down_event->getXnormalized();
  float dy =
      mouse_up_event->getYnormalized() - _mouse_down_event->getYnormalized();
...
void EarthManipulator::addMouseEvent(const osgGA::GUIEventAdapter &ea) {
  _ga_t1 = _ga_t0;
  _ga_t0 = &ea;
  //_touchPointQueue.clear();
}

void EarthManipulator::addTouchEvents(const osgGA::GUIEventAdapter &ea) {
  _ga_t1 = _ga_t0;
  _ga_t0 = &ea;
...
bool EarthManipulator::parseTouchEvents(TouchEvents &output) {
  ...
  if (_touchPointQueue.size() == 2) {
        ...
        _touchPointQueue[1].size() == 2)   // two fingers
    {
      ...
      if (p0[0].phase != osgGA::GUIEventAdapter::TOUCH_ENDED &&
        ...
        osg::Vec2f vec0 =
            osg::Vec2f(p0[1].x, p0[1].y) - osg::Vec2f(p0[0].x, p0[0].y);
        osg::Vec2f vec1 =
            osg::Vec2f(p1[1].x, p1[1].y) - osg::Vec2f(p1[0].x, p1[0].y);
...
void EarthManipulator::setByMatrix(const osg::Matrixd &matrix) {
  ...
  if (intersect(start_segment, end_segment, ip, normal)) {
    ...
    osg::Matrixd rotation_matrix =
        osg::Matrixd::translate(0.0, 0.0, -_distance) * matrix *
...
osg::Matrixd EarthManipulator::getWorldMatrix() const {
...
osg::Matrixd EarthManipulator::getWorldInverseMatrix() const {
  return osg::Matrixd::translate(-_center) *
         osg::Matrixd::rotate(_centerRotation.inverse()) *
         osg::Matrixd::translate(-_posOffset) *
         osg::Matrixd::rotate(_tetherRotation.inverse()) *
         osg::Matrixd::rotate(_rotation.inverse()) *
         osg::Matrixd::translate(-_viewOffset.x(), -_viewOffset.y(),
                                 -_distance);
}
...
void EarthManipulator::setUpdateCameraNodeVisitor(osg::NodeVisitor *nv) {
...
void EarthManipulator::setByLookAt(const osg::Vec3d &eye,
  ...
  if (_node.lock(safeNode)) {
    ...
    double maxDistance =
        distance + 2 * (eye - safeNode->getBound().center()).length();
  ...
  _centerRotation = computeCenterRotation(
      _center); // getRotation( _center ).getRotate().inverse();
...
void EarthManipulator::setByLookAtRaw(const osg::Vec3d &eye,
                                      const osg::Vec3d &center,
                                      const osg::Vec3d &up) {
  // compute rotation matrix
  osg::Vec3d lv(center - eye);
...
bool EarthManipulator::recalculateCenterFromLookVector() {
...
void EarthManipulator::recalculateCenter(const osg::CoordinateFrame &frame) {
  osg::ref_ptr<osg::Node> node;
  if (_node.lock(node)) {
    // need to reintersect with the terrain
    ...
    osg::Vec3d normal;
    bool hit_ip1 =
        intersect(_center - up * ilen * 0.1, _center + up * ilen, ip1, normal);
    bool hit_ip2 =
        intersect(_center + up * ilen * 0.1, _center - up * ilen, ip2, normal);
    if (hit_ip1) {
      if (hit_ip2) {
        setCenter((_center - ip1).length2() < (_center - ip2).length2() ? ip1
                                                                        : ip2);
      } else {
        setCenter(ip1);
      }
...
void EarthManipulator::pan(double dx, double dy) {
  if (!isTethering()) {
    ...
    if (_settings->getLockAzimuthWhilePanning()) {
      // in azimuth-lock mode, _centerRotation maintains a consistent north
      // vector
      _centerRotation = computeCenterRotation(_center);
    }
    ...
    _viewOffset.x() =
        osg::clampBetween(_viewOffset.x(), -_settings->getMaxXOffset(),
                          _settings->getMaxXOffset());
    _viewOffset.y() =
        osg::clampBetween(_viewOffset.y(), -_settings->getMaxYOffset(),
                          _settings->getMaxYOffset());
  }
...
void EarthManipulator::rotate(double dx, double dy) {
  ...
  double minp =
      osg::DegreesToRadians(osg::clampAbove(_settings->getMinPitch(), -89.9));
  double maxp =
      osg::DegreesToRadians(osg::clampBelow(_settings->getMaxPitch(), 89.9));
...
bool EarthManipulator::screenToWorld(float x, float y, osg::View *theView,
                                     osg::Vec3d &out_coords) const {
  ...
  return mapNode->getTerrain()->getWorldCoordsUnderMouse(view, x, y,
                                                         out_coords);
}

void EarthManipulator::setDistance(double distance) {
  _distance = osg::clampBetween(distance, _settings->getMinDistance(),
                                _settings->getMaxDistance());
}

void EarthManipulator::setInitialVFOV(double vfov) { _lastKnownVFOV = vfov; }

void EarthManipulator::dumpActionInfo(const EarthManipulator::Action &action,
                                      osg::NotifySeverity level) const {
  osgEarth::notify(level) << "action: " << s_actionNames[action._type]
                          << "; options: ";
  for (ActionOptions::const_iterator i = action._options.begin();
       i != action._options.end(); ++i) {
...
void EarthManipulator::handleMovementAction(const ActionType &type, double dx,
  ...
  case ACTION_EARTH_DRAG:
    if (_thrown)
      pan(dx * 0.5, dy * 0.5); // TODO: create proper drag throwing instead of
                               // panning trick
    else
      drag(dx, dy, view);
    break;
...
bool EarthManipulator::handlePointAction(const Action &action, float mx,
                                         float my, osg::View *view) {
  ...
  if (screenToWorld(mx, my, view, point)) {
    ...
    case ACTION_GOTO: {
      ...
      double range_factor =
          action.getDoubleOption(OPTION_GOTO_RANGE_FACTOR, 1.0);
...
void EarthManipulator::handleContinuousAction(const Action &action,
                                              osg::View *view) {
  double t_factor = (_time_s_now - _last_continuous_action_time) / 0.016666666;
  _last_continuous_action_time = _time_s_now;
  handleMovementAction(action._type, _continuous_dx * t_factor,
                       _continuous_dy * t_factor, view);
}

void EarthManipulator::applyOptionsToDeltas(const Action &action, double &dx,
                                            double &dy) {
...
bool EarthManipulator::handleMouseAction(const Action &action,
                                         osg::View *view) {
...
bool EarthManipulator::handleMouseClickAction(const Action &action) {
  // TODO.
  return false;
}

bool EarthManipulator::handleKeyboardAction(const Action &action,
                                            double duration) {
  double dx = 0, dy = 0;
...
bool EarthManipulator::handleScrollAction(const Action &action,
                                          double duration) {
...
bool EarthManipulator::handleAction(const Action &action, double dx, double dy,
                                    double duration) {
  bool handled = true;
  ...
  // OE_NOTICE << "action=" << action << ", dx=" << dx << ", dy=" << dy <<
  // std::endl;
...
void EarthManipulator::recalculateRoll() {
  osg::Matrixd rotation_matrix;
...
void EarthManipulator::getCompositeEulerAngles(double *out_azim,
...
void EarthManipulator::getEulerAngles(const osg::Quat &q, double *out_azim,
                                      double *out_pitch) const {
  osg::Matrix m(q);
...
osg::Quat EarthManipulator::getQuaternion(double azim, double pitch) const {
...
void EarthManipulator::collapseTetherRotationIntoRotation() {
  // fetch the composite rotation angles (_rotation and _tetherRotation):

  double azim, pitch;
  getCompositeEulerAngles(
      &azim, &pitch); // TODO replace with
                      // getEulerAngles(_rotation*_tetherRotation, ...)
...
void EarthManipulator::setHomeViewpoint(const Viewpoint &vp,
                                        double duration_s) {
  _homeViewpoint = vp;
...
bool sphereInterection(const osg::Vec3d &p0, double r0, const osg::Vec3d &p1,
                       double r1, osg::Vec3d &resultCenter, double &r) {
  using namespace osg;
  Vec3d ptvec = (p1 - p0);
...
osg::Vec3d calcTangentPoint(const osg::Vec3d &pt, const osg::Vec3d &center,
  ...
  bool valid =
      sphereInterection(center, radius, center2, radius2, resCtr, resRad);
  if (!valid)
...
osg::Matrixd rotateAroundPoint(const osg::Vec3d &pt, double theta,
  ...
  return (osg::Matrixd::translate(pt) * osg::Matrixd::rotate(theta, axis) *
...
void EarthManipulator::drag(double dx, double dy, osg::View *theView) {
  osgViewer::View *view = dynamic_cast<osgViewer::View *>(theView);
  if (!view)
    return;
  ...
  double radiusEquator =
      _srs.valid() ? _srs->getEllipsoid()->getRadiusEquator() : 6378137.0;
  ...
  bool onEarth =
      screenToWorld(_ga_t1->getX(), _ga_t1->getY(), view, worldStartDrag);
  if (onEarth) {
    ...
    if (_lastPointOnEarth != zero) {
      ...
      const osg::Vec3d startWinPt =
          getWindowPoint(view, _ga_t1->getX(), _ga_t1->getY());
      const osg::Vec3d startDrag =
          calcTangentPoint(zero, zero * viewMat, radiusEquator, startWinPt);
  ...
  if (endOnEarth) {
    ...
    const osg::Vec3d endDrag =
        calcTangentPoint(zero, earthOrigin, radiusEquator, winpt);
  ...
  if (_srs->isGeographic()) {
    ...
    osg::Matrixd Mmanip = osg::Matrixd::translate(
                              -_viewOffset.x(), -_viewOffset.y(), -_distance) *
                          viewMatInv;
