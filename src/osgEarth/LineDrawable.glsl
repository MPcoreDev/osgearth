...
void oe_LineDrawable_VS_VIEW(inout vec4 currView)
{
    ...
    vec4 currNorm=currView-earthCenterView;
    
    //we propagate the z value of the current vertex to the fragment
    oe_LineDrawable_backFaceCulled_Frag=currNorm.z;
    
    // if the current , previous and next line vertices are all behind the earth center
    if(currNorm.z<0.0 && nextNorm.z<0.0 && prevNorm.z<0.0){
        // we set the culling boolean to true and quit the function
        oe_LineDrawable_backFaceCulled=true;        
        return;
    }
    oe_LineDrawable_backFaceCulled=false;
...
{
    //the following condition enables backface culling on line drawable
    // \todo make it optional with a define
    if(oe_LineDrawable_backFaceCulled){
        //do not expand the line if it is culled
        return;
    }
...
in float oe_LineDrawable_backFaceCulled_Frag;
